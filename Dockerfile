...
RUN echo '@daily ssh -o StrictHostKeyChecking=accept-new kevin@synology.ferret-interval.ts.net docker exec -t mastodon-db pg_dumpall -c -U postgres > /backups/mastodon-db-$(date +%Y-%m-%d).sql' > /etc/crontabs/root
